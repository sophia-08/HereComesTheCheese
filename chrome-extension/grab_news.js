...
function grabHeadlines() {
  ...
  if (window.location.hostname.includes("yahoo.com")) {
    ...
    const headlineLinks = document.querySelectorAll(
      'a[class*="ntk-link"], a[data-test-locator="lead-content-link"]'
    );
    ...
    const filmstripLinks = document.querySelectorAll(
      '.ntk-filmstrip .ntk-item a[data-test-locator="item-link"]'
    );
    ...
    filmstripLinks.forEach((link, index) => {
      ...
      const h3Element = link.querySelector(
        'h3[data-test-locator="item-title"]'
      );
      if (h3Element) {
...
function filterHeadlines(headlines, searchInput) {
  ...
  const sortedResults = scoredHeadlines.sort(
