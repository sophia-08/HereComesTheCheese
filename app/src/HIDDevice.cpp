...
HIDDevice::HIDDevice(IOHIDDeviceRef device, ProxySocketServer *server)
    ...
    , m_server(server)
    , m_connected(true)
    , m_execution_engine(std::make_unique<ExecutionEngine>())  // Initialize first
{
    IOHIDDeviceOpen(m_device, kIOHIDOptionsTypeNone);
    m_ai_client = std::make_unique<OpenAIClient>(m_execution_engine.get(), system_message);
...
void HIDDevice::inputReportCallback(void *context, IOReturn result, void *sender,
