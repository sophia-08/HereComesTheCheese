...
std::string ZoomScheduler::getAccessToken() const {
  if (m_accountId.empty() || m_clientId.empty() || m_clientSecret.empty()) {
    throw std::runtime_error(
        "Missing required Zoom credentials in environment variables. "
...
ZoomScheduler::scheduleMeeting(const std::string &topic,
  ...
  auto response =
      cpr::Post(cpr::Url{baseUrl + "/users/me/meetings"},
  ...
  if (response.status_code == 201) {
    ...
    std::cerr << "Failed to schedule meeting. Status code: "
