...
void HIDManager::deviceAdded(void *context, IOReturn result, void *sender,
...
void HIDManager::deviceRemoved(void *context, IOReturn result, void *sender,
                               IOHIDDeviceRef device) {
  (void)result;
  (void)sender;
  auto *manager = static_cast<HIDManager *>(context);
...
void HIDManager::handleDeviceAdded(IOHIDDeviceRef device) {
  ...
  json j = {{"type", "device_added"}, {"message", "New HID device connected"}};
...
CFMutableDictionaryRef HIDManager::createMatchingDictionary() {
  ...
  log("Matching dictionary created for VendorID: 0x" +
      std::to_string(vendorID) + ", ProductID: 0x" + std::to_string(productID));
